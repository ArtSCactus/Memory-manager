...
_read(VA ptr, void* *pBuffer, size_t szBuffer) { // добавил * перед pBuffer, т.к. чтобы избежать return, 
	//нам необходимо работать именно с адресом, а не с локальной переменной. В случае, если мы работаем с локальной переменной, без возврата (retun pBuffer) не обойтись,
